...
- **Project Grid** — the main way to arrange the *Selected Work* row. It draws
  ...
  the row up or down. The rows glide as you move, the one you're holding lifts,
  and it saves the moment you drop. It works with a finger on a tablet as well
  as a mouse. The **⌃⌃ / ⌄⌄** row buttons still jump a project straight to the
  start or end, and the **Order** column (via "toggle columns") shows exact
  positions. Dragging is paused while a column sort or a search is active (the
  visible order isn't the real one then) — clear the sort/search to drag again.
  The Project Grid page is the one to use when you care how the row actually
  *looks*; the table is quicker for a straight top-to-bottom running order.
