...
return [
    ...
    'backup' => [
        ...
        'source' => [
            'files' => [
                /*
                 * Uploaded files only — not the whole codebase.
                 *
                 * Two reasons. First, base_path() sweeps up `.env`, which holds
                 * the database password and the Claude API key; an archive on
                 * off-server storage is exactly where those should not be.
                 * Second, code and vendor are deployment artifacts you can
                 * rebuild, whereas a scanned proforma invoice attached to a
                 * shipment exists nowhere else.
                 *
                 * Keep `.env` backed up separately and deliberately — see
                ...
                /*
                 * The backup destination and temp directory exclude themselves —
                 * BackupJob calls excludeFilesFrom() on both before selecting
                 * files, so archives do not nest inside the next archive.
                 */
                'exclude' => [],
        ...
        'destination' => [
            ...
            /*
             * A backup on the same disk as the database is not a backup — it
             * dies with the server. `local` is the development default; point
             * BACKUP_DISK at off-server storage in production.
             */
            'disks' => [
     ...
     */
    /*
     * Only the failures are worth an email.
     *
     * A nightly "backup succeeded" message trains you to ignore the sender,
     * which is precisely the sender you need to notice on the night it says
     * something else. Health is still checked — `backup:monitor` reports an
     * unhealthy backup, which covers the silent-failure case that success
     * notifications are usually meant to guard against.
     */
    'notifications' => [
        'notifications' => [
